...
- 支持枚举 Windows、macOS 和 Linux 的本机 IPv4 网络接口并自动生成 CIDR 网段。
- 默认使用系统 Ping 探测在线主机，可选叠加 TCP 端口探测。
- 支持扫描超时、并发数量、TCP 端口列表、进度展示和任务取消。
- 单次扫描最多包含 4096 个地址和 32 个 TCP 端口，避免误扫超大网段。
- 扫描结果可以直接预填 SSH、Telnet 或 Raw TCP 连接配置。
...
- [条款声明](docs/legal/terms.md)
- [安全声明](docs/legal/security.md)

关于页会通过系统默认浏览器打开以上仓库文档。
