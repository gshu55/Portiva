...
use crate::domain::connection::{DiskPartitionOverview, DiskPartitionRole};
...
mod tests {
    ...
    fn parses_wsl_host_overview_metrics() {
        let output = "portivaWslOverviewVersion\t1\n\
                      ...
                      diskUsedKb\t26214400\n\
                      rootDiskDevice\t/dev/sdc\n\
                      rootDiskMount\t/\n\
                      rootDiskFilesystem\text4\n\
                      rootDiskAvailableKb\t78643200\n\
        ...
        assert_eq!(overview.disk_partitions.len(), 1);
        assert_eq!(overview.disk_partitions[0].device, "/dev/sdc");
        assert_eq!(
            overview.disk_partitions[0].available_bytes,
            Some(80_530_636_800)
        );
